 ...
 */
export const getAgentListings = catchAsync(async (req, res, next) => {
...
export const updateListing = catchAsync(async (req, res, next) => {
    ...
    const updatedListing = await Listing.findByIdAndUpdate(listingId, { $set: updates }, {
        new: true,
        runValidators: true,
    }).lean();
    res.status(200).json({
...
export const deleteListing = catchAsync(async (req, res, next) => {
    ...
    if (listing.images && listing.images.length > 0) {
        await Promise.allSettled(listing.images.map(image => deleteImage(image.cloudinaryId).catch(error => console.error(`Failed to delete image ${image.cloudinaryId}:`, error))));
    }
...
export const getAllListings = catchAsync(async (req, res, next) => {
    ...
    const queryObject = { status: 'Active' }; // Only show active listings
    ...
    if (req.query.beds)
        queryObject.bedrooms = { $gte: Number(req.query.beds) };
    if (req.query.baths)
        queryObject.bathrooms = { $gte: Number(req.query.baths) };
...
export const getListingById = catchAsync(async (req, res, next) => {
    ...
    const listing = await Listing.findOneAndUpdate({ _id: req.params.id, status: 'Active' }, // Only show Active listings
    { $inc: { viewsCount: 1 } }, { new: true } // Return the updated document
    ).populate('agent', 'firstName lastName phone email');
    if (!listing) {
